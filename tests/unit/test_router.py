...
def test_agent_chat_with_explicit_editor_context_routes_to_mcp_readonly_tool() -> None:
    ...
    assert routing["intent"]["route_type"] == "single_tool"
    assert routing["route"]["selected_tool_id"] == "mcp_get_editor_context"
    assert routing["route"]["decision_source"] == "heuristic_task_signal"
...
def test_agent_chat_with_explicit_selected_assets_routes_to_mcp_readonly_tool() -> None:
    request = _request(
        content="List selected assets",
        context={
            "project_name": "DemoProject",
            "active_panel": "AgentChat",
        },
...
def test_agent_chat_with_named_asset_detail_routes_to_mcp_asset_details() -> None:
    request = _request(
        content="What type and path is BP_PlayerCharacter?",
        context={
            "project_name": "DemoProject",
            "active_panel": "AgentChat",
        },
    ...
    assert routing["intent"]["route_type"] == "single_tool"
    assert routing["route"]["selected_tool_id"] == "mcp_get_asset_details"
