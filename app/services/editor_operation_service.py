...
from app.services.editor_operations.catalog import (
    ASSET_NAME_RE as _ASSET_NAME_RE,
    ...
    CLASS_NAME_RE as _CLASS_NAME_RE,
    EDITOR_OPERATION_PROPOSAL_TYPE,
    MATERIAL_PARAMETER_TYPES,
    OPERATION_SPECS,
    PARAMETER_NAME_RE as _PARAMETER_NAME_RE,
    SAFE_TEXT_RE as _SAFE_TEXT_RE,
...
from app.services.editor_operations.results import (
    as_string_list,
...
class EditorOperationService:
    ...
    def _operation_group(operation_type: str) -> str:
        return operation_group(operation_type)
    ...
    def _normalize_asset_path(value: Any, *, require_game_root: bool = True) -> str:
        return normalize_asset_path(value, require_game_root=require_game_root)
    ...
    def _normalize_redirector_folder(value: Any) -> str:
        return normalize_redirector_folder(value)

    @staticmethod
    def _normalize_asset_name(value: Any, field_name: str = "asset_name") -> str:
        return normalize_asset_name(value, field_name)

    @staticmethod
    def _normalize_class_path(value: Any) -> str:
        return normalize_class_path(value)
    ...
    def _normalize_optional_string(value: Any, *, max_length: int = 120) -> str:
        return normalize_optional_string(value, max_length=max_length)
    ...
    def _build_summaries(self, operation_type: str, payload: dict[str, Any]) -> tuple[str, str]:
        return build_operation_summaries(operation_type, payload)

    @staticmethod
    def _build_affected_targets(operation_type: str, payload: dict[str, Any]) -> list[dict[str, Any]]:
        return build_affected_targets(operation_type, payload)

    @staticmethod
    def _expected_result_contract(operation_type: str) -> dict[str, Any]:
        return expected_result_contract(operation_type)
    ...
    def build_action_proposal(
        ...
        *,
        ...
        return build_editor_operation_action_proposal(
            ...
            policy_reason=request.reason or "",
        )
    ...
    def _collection_count(value: Any) -> int:
        return collection_count(value)

    @staticmethod
    def _first_non_empty_text(*values: Any) -> str:
        return first_non_empty_text(*values)
    ...
    def _blueprint_graph_result_diagnostics(
        *,
        ...
        return blueprint_graph_result_diagnostics(
            request=request,
            preview=preview,
            result=result,
    ...
    def _blueprint_graph_repair_advice(
        *,
        ...
        return blueprint_graph_repair_advice(
            operation_type=operation_type,
    ...
    def _first_node_identifier(value: Any) -> str:
        return first_node_identifier(value)

    @staticmethod
    def _entry_event_node_hint(entry_event: str) -> str:
        return entry_event_node_hint(entry_event)
    ...
    def operation_follow_up_candidates(self, proposal_id: str) -> dict[str, Any] | None:
        ...
        return operation_follow_up_payload(
            proposal_id=proposal_id,
            preview=preview,
            is_editor_operation=proposal.proposal_type == EDITOR_OPERATION_PROPOSAL_TYPE,
        )
    ...
    def _redirector_follow_up_folders(
        *,
        ...
        return redirector_follow_up_folders(
            operation_type=operation_type,
            payload=payload,
            result=result,
        )
    ...
    def _follow_up_quick_actions(*, proposal_id: str, follow_up: dict[str, Any]) -> list[dict[str, Any]]:
        return follow_up_quick_actions(proposal_id=proposal_id, follow_up=follow_up)
    ...
    def _operation_result_user_view(
        *,
        ...
        return operation_result_user_view(
    ...
    def prepare_follow_up_proposal_request(
        *,
        ...
        return materialize_follow_up_proposal_request(
